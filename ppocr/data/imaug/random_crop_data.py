...
class EastRandomCropData(object):
    ...
    def __call__(self, data):
        ...
        scale = 1
        h,w = self.size[0], self.size[0]

        h_list = np.arange(0,img_h,h)
        w_list = np.arange(0,img_w,w)

        crop_x, crop_y, crop_w, crop_h = np.random.choice(w_list), np.random.choice(h_list),\
                                         w,h
        #print(crop_x,crop_y)
        if self.keep_ratio:
            padimg = np.zeros((self.size[1], self.size[0], img.shape[2]),
                              img.dtype)
            padimg[:np.min([crop_y + crop_h,img_h])-crop_y, :np.min([crop_x + crop_w,img_w])-crop_x] = img[crop_y:np.min([crop_y + crop_h,img_h]), crop_x:np.min([crop_x + crop_w,img_w])]
